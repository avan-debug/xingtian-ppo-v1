...
def get_bm_fix_path(bm_info, key_seq, last_path=None):
    """Get model path of benchmark template."""
...
def main():
    ...
    parser.add_argument(
        ...
        help="""config file with template""",
    )
    parser.add_argument(
    ...
    if args.case in ("ALL", "all",):
        ...
        if "," in args.case:
            ...
            files_candi = [
                os.path.join(args.yaml_path, "{}.template".format(_yaml))
                for _yaml in case_list
            ]
        else:
            files_candi = [os.path.join(args.yaml_path, "{}.template".format(args.case))]
    else:
    ...
    for config_file in bm_cases:
        ...
        print("run with template: {}".format(to_yaml))
        sys.stdout.flush()
